...
if (@$_SERVER[ENV_PRE.'DEV_MODE'] === 'true') {
    ...
    return array(
        ...
        'DB_NAME'   => $_SERVER[ENV_PRE.'DB_NAME'] ? : 'dtadmin',       // 数据库名
        'DB_USER'   => $_SERVER[ENV_PRE.'DB_USER'] ? : 'root',       // 用户名
        'DB_PWD'    => $_SERVER[ENV_PRE.'DB_PWD']  ? : '',        // 密码
        'DB_PORT'   => $_SERVER[ENV_PRE.'DB_PORT'] ? : '[DB_PORT]',       // 端口
        'DB_PREFIX' => $_SERVER[ENV_PRE.'DB_PREFIX'] ? : 'dt_',   // 数据库表前缀
        'AUTH_KEY'  => '$[Z<QRs`!&w-:%pX)%qnBiVJH{f&Zj?=~M,DQ~,Kw|$%jvT{"cBNzzr]W(MV!-P_', // 系统加密KEY，轻易不要修改此项，否则容易造成用户无法登录，如要修改，务必备份原key
    );
} else {
    // 数据库配置
    return array(
